...
import { earningsApi, eventsApi, usersApi } from "../../shared/services/api";
import Swal from "sweetalert2";

export default function EventEarn() {
  ...
  const eventColumns = [
    ...
    { field: "paid", headerName: "Paid", flex: 1, minWidth: 90 },
    { field: "due", headerName: "Due", flex: 1, minWidth: 90 },
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const eventEarningsResponse = await usersApi.getUserEventEarnings();
        setEventEarnings(eventEarningsResponse.data);
        console.log(eventEarningsResponse);
      } catch (error) {
        ...
        >
          {loading ? (
            ...
            <div className="flex items-center justify-center h-40">
              <span>No event earnings found</span>
            </div>
          ) : (
            <DataGrid
              rows={eventEarnings}
