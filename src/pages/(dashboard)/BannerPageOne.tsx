      ...
      <div className="flex-1 py-8 px-5 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className={`flex-1 py-5 px-3 rounded-xl ${theme === 'dark' ? 'bg-[#121212] ' : 'bg-white'}`}>
            <h2 className="text-lg font-semibold mb-4 pt-1 px-3 pb-1">Select Events</h2>
            <div className="grid lg:grid-cols-4  md:flex-row gap-3 ">
              {loading ? (
                ...
                <div key={index} className={` p-4 rounded-xl ${theme === 'dark' ? 'bg-[#121212] shadow-[0_0px_4px_rgba(255,255,255,0.2)]' : 'bg-gray-100  '}`} >
                  <img src={event.event_img} alt="Event" className="w-full h-24 object-cover rounded-xl mb-4" />
                  <Link onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
                    to={'/dashboard/event/poster/' + event.event_id} >

                    <div className={`text-xl font-medium text-center  mt-1 ${theme === 'dark' ? 'text-white' : 'text-[#040171]'}`}>{event.event_title}</div>
                  </Link>
