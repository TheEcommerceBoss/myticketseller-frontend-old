...
const PaymentSettings = () => {
			...
			try {
				...
				if (event.days && event.days.length > 0) {
					...
					const formattedEventDays = event.days.map((day, index) => ({
						...
						location:
							day.event_type == "onsite" ? day.event_address : "",
						virtualLink:
							day.event_type == "virtual" ? day.event_link : "",
						password:
	...
	const submitPayment = async () => {
		...
		for (const ticket of tickets) {
			if (!ticket.name) {
				Swal.fire(
					"Error",
					"Please complete the ticket name for ticket " +
						ticket.id +
						".",
					"error"
				);
		...
		for (const day of eventDays) {
			if (!day.startDate) {
				Swal.fire(
					"Error",
					"Please complete the start date for the event day " +
						day.index +
						".",
					"error"
				);
			...
			if (!day.startTime) {
				Swal.fire(
					"Error",
					"Please complete the start time for the event day " +
						day.index +
						".",
					"error"
				);
			...
			if (!day.endTime) {
				Swal.fire(
					"Error",
					"Please complete the end time for the event day " +
						day.index +
						".",
					"error"
				);
	...
	const addTicket = (type) => {
		if (
			...
			Swal.fire(
				"Error",
				"You can only create one Guest List ticket!",
				"error"
			);
			return;
		}
					...
					<div className="space-y-6">
						...
						{tickets.map((ticket) => {
							...
							return (
								<div
									key={ticket.id}
									className="p-4 mb-8 border border-gray-500 rounded"
								>
									<h3 className="my-2">
										{`Ticket 1 (${ticket.type})`}
									</h3>
									...
									<div className="mb-4">
										<div className="flex gap-5">
											<button
												...
												className="items-center hidden px-4 py-1 mt-2 text-sm text-white transition bg-gray-500 rounded hover:bg-gray-600"
											>
												<Plus
													size={17}
													className={"mr-2"}
												...
												<select
													...
													className={`flex ${
														theme == "dark"
															? "bg-transparent"
															: "border border-[#A2A2A2]"
													}  p-1 w-full p-2 border border-[#A2A2A2] rounded-lg py-4 text-l`}
												>
													<option value="onsite">
														On Site
													</option>
