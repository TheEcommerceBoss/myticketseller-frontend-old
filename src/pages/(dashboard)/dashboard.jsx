...
import { dashboardApi, usersApi } from "../../shared/services/api/index";
			...
			try {
				...
				const paddedData = requiredMonths.map((required) => {
					const eventData = transformedEventData.find(
						(entry) =>
							entry.month === required.month &&
							entry.year === required.year
					);
				...
				<div className="flex items-center justify-between mb-8">
					<div className="flex items-center space-x-4">
						...
						<div className="">
							...
							>
								<Greetings />, {userDetails.full_name}
							</span>
						</div>
					</div>
				...
				{title == "Balance" ? (
					<button className="cursor-pointer">
						<Eye
							...
							onClick={() =>
								setBalance_hidden(Balance_hidden ? false : true)
							}
						/>
					</button>
				) : (
