  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        eventData.event_password && setEventPassword(eventData.event_password);

        eventData.restrictions && setSelectedAudience(eventData.restrictions);
        ...
        const authoritiesArray = specialGuests.map((item) => item.trim());
              ...
              <div className="flex items-center mb-[2rem] mt-2">
                <div
                  className={`w-5 h-5 rounded-full bg-transparent ${
                    theme === "dark" ? "border-gray-200  " : "border-[#040171]"
            ...
            <div className="flex items-center justify-center">
              <button
                onClick={() => {
                  ...
                  setGuestNames((prev) =>
                    prev.slice(0, Math.max(0, guestCount - 1))
                  );
                }}
                className="w-[5rem] h-12 bg-[#040171] text-white rounded-l-full flex items-center justify-center text-l"
              >
          ...
          >
            ...
            {notifyAuthorities && (
              <div className="flex flex-col items-start md:grid md:grid-cols-3 mt-[3rem] text-center">
                {["Police", "Fire Department", "Traffic Control"].map(
                  (authority, index) => (
